...
export function QuoteCta() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(
        'Something went wrong while sending your request. Please try again or give us a call.'
      )
    } finally {
      setSubmitting(false)
    }
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className={FIELD}
                  placeholder="Bedrooms, stairs, fragile items…"
                />
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="mt-1 inline-flex items-center justify-center rounded-md bg-accent px-6 py-3.5 text-base font-bold text-accent-foreground transition-transform hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
              >
                {submitting ? 'Sending...' : 'Get My Free Quote'}
              </button>

              <p className="text-center text-xs text-muted-foreground">
                We respect your privacy. Your details are only used to prepare
                your quote.
              </p>
            </form>
          )}
